...
public class SynchBarVertical extends OldGridElement {
	...
	public void paintEntity(Graphics g) {
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			if (getRectangle().getWidth() / 2 - width / 2 < 0) {
				ADAPT_SIZE = true;
				break;
			}
		...
		g2.fillRect(getRectangle().getWidth() / 2 - (int) (3 * zoom), textHeight + (int) HandlerElementMap.getHandlerForElement(this).getFontHandler().getDistanceBetweenTexts(), (int) (5 * zoom), getRectangle().getHeight() - textHeight - (int) HandlerElementMap.getHandlerForElement(this).getFontHandler().getDistanceBetweenTexts() * 2);
	}
